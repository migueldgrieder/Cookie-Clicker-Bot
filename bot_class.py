...
class CookieClickerBot:
    ...
    def check_unlock_next_farm(self):
        ...
        if owned_qtd_html.text == "1":
            productOwned_farm = str(int(self.productOwned_farm)+1)
            self.farms = [self.browser.find_element_by_id("productPrice" + str(i)) for i in range(productOwned_farm,-1,-1)]
        pass
    ...
    def controller(self):
        ...
        count = int(self.cookie_count.text.split(" ")[0])
        len_farms = len(self.farms)


        for i in range(len_farms):
            value_farm = int(self.farms[i].text) 
            if i > 0:
                value_next_farm = int(self.farms[i-1].text) 
